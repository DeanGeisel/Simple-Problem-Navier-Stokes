...
ymesh=33; %number of evenly spaced y values to be used in the approximation
xmesh=129; %number of evenly spaced x values to be used in the 
N=ymesh*xmesh; %abbreviation for common used number
w=0.5; %half of the width of the surface, perpendicular to the flow.
...
eta=0.1 ;%1/Reynold's number /Assume Reynolds number of 200 in this case. 
...
for p=2:xmesh-1
    for q=2:ymesh-1
        L=rho*k*UM(q,p)*(UM(q,p)-UM(q,p-1));
...
%F left border (initial x, not corners)
 ...
 for p=2:xmesh-1
    ...
    L=rho*k*UM(1,p)*(UM(1,p)-UM(1,p-1));
 ...
 for p=2:xmesh-1
    ...
    L=rho*k*UM(ymesh,p)*(UM(ymesh,p)-UM(ymesh,p-1));
 ...
 for q=2:ymesh-1
    ...
    L=rho*k*UM(q,xmesh)*(UM(q,xmesh)-UM(q,xmesh-1));
...
for p=2:xmesh
    for q=1:ymesh
        Jhold(q,p)=rho*k*(2*UM(q,p)-UM(q,p-1))+4*eta;
    end
end

for q=1:ymesh
    Ugst=2*UM(q,1)-UM(q,2);
    Jhold(q,1)=rho*k*(2*UM(q,1)-Ugst)+4*eta;
end


JV=reshape(Jhold,N,1);

%Forming the left diagonal dependent upon U
Lhold=zeros(ymesh,xmesh-1);
for p=2:xmesh
    for q=1:ymesh
        Lhold(q,p-1)=-rho*k*(UM(q,p))-eta;
    end
end

LV=reshape(Lhold,ymesh*(xmesh-1),1);

%Assembling the Jacobian

I=eye(xmesh);
IJV=spdiags(JV,0,N,N);
ILV=spdiags(LV,-ymesh,N,N);
e=ones(ymesh,1);
ek=ones(xmesh,1);
